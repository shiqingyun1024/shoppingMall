angular.module('goodsList_service', [])
  .factory('goodsListFactory', function($http,$q,GlobalVariable) {
    return {
      // 刷新列表商品列表
      ...
      getAllData:function(){
        ...
        // $http的get请求
        $http.get("localhost:8081/areas/78").success(function(data,status,config,headers){
        ...
        // $http的post请求
        $http.post("localhost:8081/areas",{typeNumber:10004}).success(function(data,status,config,headers){
        ...
        // 拼接数据请求访问地址
        var url=GlobalVariable.SERVER_PATH+"/areas?message=pms&callback=JSON_CALLBACK";
        // 通过jsonp方式进行跨域访问
